...
export default function BuyerScanner() {
  ...
  // Current user state from profiles table
      ...
      if (user) {
        ...
        if (error) {
          ...
          setCurrentUser({
            ...
            wallet: profile.wallet_address || account,
            email: profile.email || ''
          })
        }
      } else {
  ...
  // Updated processCheckout with buyer name
  async function processCheckout() {
    ...
    try {
      ...
      const orderData = {
        ...
        buyer_name: currentUser.name,  // ← BUYER NAME FROM PROFILE
      ...
      console.log('Saving order with buyer name:', orderData.buyer_name)
      ...
      console.log('✅ Order saved with buyer name:', data[0].buyer_name)
      ...
      <div className="buyer-page" style={{ fontFamily: "'Inter', sans-serif", minHeight: '100vh', background: '#060c04', position: 'relative', overflow: 'hidden' }}>
        ...
        <nav style={{ position: 'sticky', top: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', height: '60px', background: 'rgba(4,9,2,0.85)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(96,165,250,0.10)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 12px', background: 'rgba(96,165,250,0.06)', border: '1px solid rgba(96,165,250,0.18)', borderRadius: '4px' }}>
              <div style={{ width: '24px', height: '24px', borderRadius: '50%', background: '#60a5fa20', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: '12px' }}>👤</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <span style={{ fontSize: '10px', color: '#60a5fa', fontWeight: 'bold' }}>{currentUser.name}</span>
                <span style={{ fontSize: '8px', color: 'rgba(255,255,255,0.3)', fontFamily: 'monospace' }}>{currentUser.wallet?.slice(0, 10)}...</span>
              </div>
            </div>
          ...
          <div style={{ marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              ...
              <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.3)', marginTop: '8px' }}>
                Welcome back, <span style={{ color: '#4ade80', fontWeight: 'bold' }}>{currentUser.name}</span>! Add items to cart and create orders.
              </p>
              ...
              <div style={{
                ...
                {buyerNotifications.length === 0 ? (
                  <div style={{
                    ...
                    <div style={{ fontSize: '48px', marginBottom: '16px' }}>🔕</div>
                    <p>No notifications yet, {currentUser.name}</p>
