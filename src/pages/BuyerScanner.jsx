...
export default function BuyerScanner() {
  ...
  // QR Scanner Modal state (for after-delivery verification)
  ...
  // Process checkout - Create PENDING order (no payment yet)
  // Process checkout - Save order to Supabase
// Process checkout - Create PENDING order (no payment yet)
// Process checkout - Save ONLY to localStorage (reliable)
// Process checkout - Save order to Supabase database
async function processCheckout() {
  ...
  try {
    ...
    showNotification('Order created! Go to "My Orders" to complete payment.', '#4ade80')
  } catch (error) {
  ...
  return (
    <>
      ...
      <div className="buyer-page" style={{ fontFamily: "'Inter', sans-serif", minHeight: '100vh', background: '#060c04', position: 'relative', overflow: 'hidden' }}>
        ...
        {/* Checkout Modal - Creates PENDING order */}
        {showCheckout && (
        ...
        {/* After-Delivery QR Verification Modal */}
        {showQrModal && (
