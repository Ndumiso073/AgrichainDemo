...
export default function BuyerOrders() {
  ...
  // Load orders and notifications from Supabase database
  useEffect(() => {
    loadOrders()
    loadNotifications()
  ...
  async function loadOrders() {
    try {
      ...
      // Check for newly approved orders and show notification
      const newlyApproved = transformedOrders.filter(o => 
        o.status === 'Approved' && o.deliveryDate && 
        !localStorage.getItem(`notified_${o.id}`)
      )
      
      newlyApproved.forEach(order => {
        showNotificationMessage(`📦 Order #${order.id} approved! Delivery scheduled for ${new Date(order.deliveryDate).toLocaleDateString()}. Please complete payment when you receive the product.`)
  ...
  // Load notifications for this buyer
  async function loadNotifications() {
    try {
      ...
      if (!error && data) {
        ...
        if (unreadNotifications.length > 0) {
          showNotificationMessage(unreadNotifications[0].message)
        }
      }
    } catch (error) {
  ...
  function showNotificationMessage(message) {
    ...
    notification.style.cssText = `
      position: fixed; bottom: 20px; left: 20px; background: #4ade80;
      color: #060c04; padding: 12px 20px; border-radius: 8px;
      z-index: 10000; animation: fadeOut 3s ease forwards;
      font-weight: bold; font-size: 14px; max-width: 400px;
    `
      ...
      <div className="orders-page" style={{
        ...
        <main style={{ position: 'relative', zIndex: 1, maxWidth: '1000px', margin: '0 auto', padding: '40px 24px 80px' }}>
          ...
          {notifications.length > 0 && (
            <div style={{ marginBottom: '32px', padding: '16px', background: 'rgba(96,165,250,0.05)', borderRadius: '12px', border: '1px solid rgba(96,165,250,0.2)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                <span style={{ fontSize: '18px' }}>🔔</span>
                <span style={{ fontSize: '12px', color: '#60a5fa', fontWeight: 'bold' }}>RECENT UPDATES</span>
              </div>
              <div style={{ maxHeight: '150px', overflowY: 'auto' }}>
                {notifications.slice(0, 3).map(notif => (
                  <div key={notif.id} style={{ fontSize: '12px', color: 'rgba(255,255,255,0.7)', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    {notif.message}
                  </div>
                ))}
              </div>
            ...
            orders.map(order => (
              ...
              >
                ...
                <div style={{
                  ...
                  <div>
                    <span style={{
                      ...
                      fontSize: '11px',
                      fontWeight: 'bold',
                      background: order.status === 'Paid' ? 'rgba(74,222,128,0.2)' : order.status === 'Cancelled' ? 'rgba(248,113,113,0.2)' : order.status === 'Approved' ? 'rgba(74,222,128,0.2)' : 'rgba(250,204,21,0.2)',
                      color: order.status === 'Paid' ? '#4ade80' : order.status === 'Cancelled' ? '#f87171' : order.status === 'Approved' ? '#4ade80' : '#facc15'
                    }}>
                ...
                <div style={{ padding: '20px' }}>
                  ...
                  {/* Delivery Date Display - NEW SECTION */}
                  {order.deliveryDate && order.status === 'Approved' && (
                    <div style={{
                      ...
                      <div style={{ fontSize: '11px', color: '#4ade80', marginBottom: '4px' }}>📦 Your order has been approved!</div>
                      <div style={{ fontSize: '13px', color: '#fff' }}>
                        Expected Delivery Date: <strong>{new Date(order.deliveryDate).toLocaleDateString()}</strong>
                      </div>
                      <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', marginTop: '8px' }}>
                        Please complete payment when you receive the product by scanning its QR code.
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ marginTop: '20px', display: 'flex', gap: '12px' }}>
                    {order.status !== 'Paid' && order.status !== 'Cancelled' && order.status === 'Approved' && (
                      <button
